...
if ( ! class_exists( 'BP_Course_Rest_Course_Controller' ) ) {
	...
	class BP_Course_Rest_User_Controller extends BP_Course_Rest_Controller {
		...
		register_rest_route( $this->namespace, '/'. $this->type .'/alluser', array(
			...
			//Pmpro level purchase from wallet
			register_rest_route( $this->namespace, '/'. $this->type .'/pmpro-level-purchase', array(
				array(
					'methods'                   =>  'POST',
					'callback'                  =>  array( $this, 'pmpro_level_purchase' ),
					'permission_callback' => array( $this, 'get_user_permissions_check' ),
				),
			));
       ...
       function get_comment_child($comment_id)
       {
		...
		{    
           ...
           **/
            ...
            $submitted = get_post_meta($assignment_id,$this->user_id,true);
            if(empty($submitted)){
				do_action('wplms_submit_assignment',$assignment_id,$this->user_id,$this->user_id);
			}
            return 	new WP_REST_Response( $data, $response );
	...
	function get_forums($request){
		...
		function set_registrationId_push_notification($request){
	    	...
	    	if(!empty($post['registrationId'])){
				...
				$data =array(
					'status' => 1,
					'message' => _x('Device registered for Push Notification','Device registered for Push Notification','vibe')
				);
	    	}else{
	    		$data = array(
	    			'status' => 0,
	    			'message' => _x('RegistrationId not found please Re-open app again','RegistrationId not found please Re-open app again','vibe')
	    		);
	    	}
		...
		function delete_registrationId_push_notification($request){
	    	...
	    	if(!empty($post['registrationId'])){
				...
				$data =array(
					'status' => 1,
					'message' => _x('Device removed for Push Notification','Device removed for Push Notification','vibe')
				);
	    	}else{
	    		$data = array(
	    			'status' => 0,
		...
		function pmpro_level_purchase($request){
			$post = json_decode(file_get_contents('php://input'));
	    	$post = json_decode(json_encode($post),true);
	    	$pmpro_level_id = $post['pmpro_level_id'];
			$data = array();
			//wallet check
			//membership check if not exist already 
			//create order 
			//membership enroll 
			//course assign(if need) 
			//wallet deduct
			$flag = apply_filters('wplms_pmpro_level_purchase_flag',true,$request);
			if($flag){
				if(!empty($pmpro_level_id)){
					...
					if(!empty($level)){
						$wallet_amount = get_user_meta($this->user_id,'wallet',true); //Amount
						$initial_payment = $level->initial_payment;
						if($initial_payment<=$wallet_amount){
							//check pmpro activated
							if(function_exists('pmpro_hasMembershipLevel')){
								if(!pmpro_hasMembershipLevel($pmpro_level_id, $this->user_id)){
									if(pmpro_changeMembershipLevel($pmpro_level_id, $this->user_id)){
										$content =  sprintf(_x('Wallet transaction worth %d for Pmpro level:%s','wallet','vibe'),$initial_payment,$level->name);
										//Create pmpro order
										$morder = new MemberOrder();
										$morder->user_id = $this->user_id;
										$morder->membership_id = $pmpro_level_id;
										$morder->InitialPayment = sanitize_text_field($initial_payment);	//not the initial payment, but the class is expecting that
										$morder->PaymentAmount = sanitize_text_field($initial_payment);
										$morder->datetime = sanitize_text_field(time());
										$morder->FirstName = sanitize_text_field(bp_core_get_user_displayname($this->user_id));
										$morder->LastName = sanitize_text_field(bp_core_get_user_displayname($this->user_id));
										$u = get_userdata($this->user_id); // get userdata
										$morder->Email = sanitize_text_field($u->user_emai);
										$morder->gateway = 'App Wallet';
										$morder->notes = $content;  //notes
										$morder->saveOrder(); //save
										$transaction_id = $morder->getMemberOrderByID( $morder->id );
										//email the user their invoice
										$pmproemail = new PMProEmail();
										$pmproemail->sendInvoiceEmail( get_userdata( $last_order->user_id ), $morder );

										//Ammount deducted and Activity add
										$wallet_amount = $wallet_amount - $initial_payment;
										update_user_meta($this->user_id,'wallet',$wallet_amount); //Amount	
										...
										$post = array(
											...
											'pid' => 'Level Purchase',
											'store' => 'wallet',
											'amount' => (int)$initial_payment,
											'points' => (int)$initial_payment,
											'description' => $content,
											'transactionid' => $transaction_id
										);
										bp_activity_update_meta($activity_id,'transaction',$post);
										do_action( 'wplms_pmpro_level_purchase',$request);
										// do here course assign if need after level purchase
										$data = array(
											'status' => 1,
											'message' => _x('Added in membership and Money Deducted','Added in membership and Money Deducted','vibe')
										);
									}else{
										$data = array(
											'status' => 0,
											'message' => _x('Not Added in membership','Not Added in membership','vibe')
										);
									}
								}else{
									$data = array(
										'status' => 0,
										'message' => _x('Already Member','Already Member','vibe')
									);
								}	
							}else{
								$data = array(
									'status' => 0,
									'message' => _x('Pmpro Not Active','Pmpro Not Active','vibe')
								);
							}
						}else{
							$data = array(
								'status' => 0,
								'message' => _x('Insufficient balance','Insufficient balance','vibe')
							);
						}
					}else{
						$data = array(
							'status' => 0,
							'message' => _x('Level not found','Level not found','vibe')
						);
					}
				}else{
					$data = array(
				...
				$data = array(
					'status' => 0,
					'message' => _x('Pmpro-Level App purchase disabled','Pmpro-Level App purchase disabled','vibe')
				);
			}
			$data = apply_filters('wplms_pmpro_level_purchase',$data,$request);
	    	return new WP_REST_Response($data, 200); 
		}
