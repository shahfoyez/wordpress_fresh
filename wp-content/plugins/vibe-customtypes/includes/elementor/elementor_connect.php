...
add_filter('wplms_enqueue_images_loaded_isotopes_page_builder',function ($flag){
	global $post;
	if(!empty($post) && !empty($post->ID) && class_exists('Elementor\Plugin') && !empty(Elementor\Plugin::instance()->db) && method_exists(Elementor\Plugin::instance()->db, 'is_built_with_elementor') && Elementor\Plugin::instance()->db->is_built_with_elementor( $post->ID )){
		return 1;
	}
	return $flag;
});
