			...
			foreach($this->settings as $key=>$setting){
				switch($key){
					...
					case 'course_coming_soon':
						...
						add_filter('wplms_take_this_course_button_label',array($this,'coming_soon_display'),9999,2);
					break;
	...
	function show_assignments_in_units($lesson){
		...
		if(isset($assignments) && is_array($assignments) && count($assignments)){
			foreach($assignments as $assignment){
				...
				$assignment_duration_parameter = intval($assignment_duration_parameter);
